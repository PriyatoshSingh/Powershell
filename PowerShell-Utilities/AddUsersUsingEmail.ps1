...
$csv = Read-Host "Enter the CSV full path, ensure there is only column named Email"
$csv = $csv.Trim('"')
...
foreach ($user in $users) {
    try {
        $graphUser = Get-MgUser -Filter "mail eq '$($user.Email)'" -ErrorAction Stop
        Write-Host "$($graphUser.Count) user(s) found for email $($user.Email)"
    } catch {
        Write-Host "Failed to retrieve user for $($user.Email). $_"
        $results += [PSCustomObject]@{
            Email    = $user.Email
            ObjectId = ''
            Status   = 'Failed'
            Message  = "Failed to retrieve user(s): $_"
        }
        continue
    }
    ...
    if ($null -ne $graphUser) {
        try {
            ...
            Write-Host "$($graphUser.DisplayName) with email $($user.Email) added to the group"
            $count++
            $results += [PSCustomObject]@{
                Email    = $user.Email
            ...
            Write-Host "$($graphUser.DisplayName) with email $($user.Email) failed to add. $_"
            $results += [PSCustomObject]@{
                Email    = $user.Email
                ObjectId = $graphUser.Id
                Status   = 'Failed'
