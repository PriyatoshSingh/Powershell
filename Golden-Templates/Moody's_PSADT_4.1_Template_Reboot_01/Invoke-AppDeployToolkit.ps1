...
function Install-ADTDeployment
{
    ...
    Invoke-MDYManagedRebootDeferral @ManagedRebootDeferralParams
    ...
    $adtSession.InstallPhase = "Post-$($adtSession.DeploymentType)"
       ...
       #Perform Action for Every User Profile
        ...
        ##Restart Computer with BitLocker Condition

    $LoggedOnUser = Get-ADTLoggedOnUser | Where-Object { $_.IsActiveUserSession -eq $true } | Select-Object -First 1
    $BLInfo = Get-BitLockerVolume -MountPoint $env:SystemDrive -ErrorAction SilentlyContinue
    $BLCheck = $BLInfo.ProtectionStatus

    if ($LoggedOnUser)
    {
        Write-ADTLogEntry -Message "An active logged-on user was detected: [$($LoggedOnUser.NTAccount)]."

        if ($BLCheck -eq 'On')
        {
            try
            {
                Write-ADTLogEntry -Message "BitLocker status is ON."
                Write-ADTLogEntry -Message "Suspending BitLocker on [$env:SystemDrive] with a reboot count of 1."
                Suspend-BitLocker -MountPoint $env:SystemDrive -RebootCount 1 -ErrorAction Stop
                Write-ADTLogEntry -Message "Successfully suspended BitLocker on [$env:SystemDrive] with a reboot count of 1."
            }
            catch
            {
                Write-ADTLogEntry -Message "Failed to suspend BitLocker on [$env:SystemDrive]. Error: $($_.Exception.Message)"
            }
        }
        else
        {
            Write-ADTLogEntry -Message "BitLocker status is OFF. BitLocker suspension is not required."
        }

        try
        {
            Close-ADTInstallationProgress -ErrorAction SilentlyContinue
            Write-ADTLogEntry -Message "Starting forced restart of the computer."
            Start-ADTProcess -FilePath "$env:SystemRoot\System32\shutdown.exe" -ArgumentList '/g /t 00 /f' -NoWait
            Write-ADTLogEntry -Message "Successfully initiated the forced restart."
        }
        catch
        {
            Write-ADTLogEntry -Message "Failed to initiate the forced restart. Error: $($_.Exception.Message)"
            throw
        }
    }
    else
    {
        Write-ADTLogEntry -Message "No active logged-on user was detected."
        ...
        if ($BLCheck -eq 'On')
        {
            ...
            try
            {
                Close-ADTInstallationProgress -ErrorAction SilentlyContinue
                Write-ADTLogEntry -Message "Starting forced restart of the computer."
                Start-ADTProcess -FilePath "$env:SystemRoot\System32\shutdown.exe" -ArgumentList '/t 00 /f' -NoWait
                Write-ADTLogEntry -Message "Successfully initiated the forced restart."
            }
            catch
            {
