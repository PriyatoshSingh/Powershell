...
function Invoke-MDYManagedRebootDeferral
{
    <#
    .SYNOPSIS
        Provides the Moody's managed in-process deferral workflow for
        reboot-required application installations.

    .DESCRIPTION
        Displays the managed deferral prompt regardless of whether any
        configured application process is currently running.

        The initial prompt provides:
        - ✔Install Now
        - ⏰Remind in 2 Hours
        - ⏰Remind in 4 Hours

        After the first deferral, subsequent prompts provide:
        - ✔Install Now
        - ⏰Remind in 2 Hours

        The final installation deadline and selected reminder details are
        stored in the native PSADT defer-history registry.

        The deployment remains active during the selected deferral period
        and does not exit with code 1602.

        If the user selects Install Now, configured AppProcessesToClose
        processes are closed silently before installation continues.

        When the final deadline expires, the native PSADT welcome dialog
        displays the final application-close countdown.

        If the deployment is running in Silent or NonInteractive mode,
        the function skips the user prompt and continues with installation.

    .PARAMETER ADTSession
        Active PSADT deployment session.

    .PARAMETER DeferDeadlineHours
        Total deferral window in hours.

    .PARAMETER InitialTwoHourDeferHours
        Duration of the initial two-hour reminder option.

    .PARAMETER InitialFourHourDeferHours
        Duration of the initial four-hour reminder option.

    .PARAMETER SubsequentDeferHours
        Duration of subsequent reminder options.

    .PARAMETER PostDeadlineCloseCountdownSeconds
        Final application-close countdown after the deadline expires.

    .PARAMETER DeferralSleepCheckSeconds
        Maximum number of seconds between reminder-time checks.

    .NOTES
        This function keeps the deployment process running during deferral.

        Configure the Intune Win32 app installation timeout to accommodate:

        - The complete deferral window
        - The final application-close countdown
        - Installation time
        - Restart processing

        Save the Extensions module as UTF-8 with BOM to preserve the
        button symbols under Windows PowerShell 5.1.
    #>
    ...
    try
    {
        ...
        {
            Write-ADTLogEntry -Message "The managed reboot-deferral workflow cannot display user prompts because the deployment is running in [$CurrentDeployMode] mode. Continuing without deferral."

            return
        }

        Write-ADTLogEntry -Message "Starting the managed reboot-deferral workflow in [$CurrentDeployMode] mode."

        ##================================================
        ## MARK: Configure Button Labels
        ##================================================

        $InstallNowButtonText = '✔Install & Reboot'
        ...
        ##================================================
        ## MARK: Create or Restore Final Deadline
        ##================================================
        ...
        if ($DeferHistory -and $DeferHistory.DeferDeadline)
        {
            try
            {
                ...
                Write-ADTLogEntry -Message "Reusing the existing final installation deadline [$DeferDeadline]."
            }
            catch
            {
                ...
                Write-ADTLogEntry -Message "The stored final installation deadline was invalid. Created a new deadline [$DeferDeadline]."
            }
        }
        else
        {
            ...
            Write-ADTLogEntry -Message "Created the initial final installation deadline [$DeferDeadline]."
        }

        ##================================================
        ## MARK: Restore Existing Reminder State
        ##================================================
        ...
        )
        {
            ...
            try
            {
                ...
                {
                    Write-ADTLogEntry -Message "Restored the existing reminder time [$NextPromptTime] from the PSADT defer-history registry."
                }
            }
            catch
            {
                ...
                Write-ADTLogEntry -Message "The existing reminder information could not be restored. Error: $($_.Exception.Message)"
            }
        }
        ...
        while (-not $ProceedWithInstallation)
        {
            ...
            )
            {
                ...
                Write-ADTLogEntry -Message "The deployment is deferred until [$NextPromptTime]. Approximately [$RemainingSeconds] seconds remain."
            ...
            {
                Write-ADTLogEntry -Message "The final installation deadline [$DeferDeadline] has expired. Displaying the final application-close countdown of [$PostDeadlineCloseCountdownSeconds] seconds."
                ...
                $FinalWelcomeParams = @{
                    ...
                    Title                   = "$($ADTSession.AppName) Install"
                    Subtitle                = "Moody's - App Installation: Reboot required"
                }
            ...
            {
                $ProcessDescriptions = 'No application closure is configured.'
            }
            ...
            {
                $Message = @"
⚠ IMPORTANT: This install requires a device restart to complete.
...
After the deadline is reached, installation and restart will be automatically enforced.
...
"@
                ...
                $PromptParams = @{
                    ...
                    Subtitle         = "Moody's - App Installation: Reboot required"
                    Message          = $Message
                    ButtonLeftText   = $InstallNowButtonText
                ...
                {
                    $InstallNowButtonText
                    {
                        Write-ADTLogEntry -Message "The user selected [$InstallNowButtonText]."
                    ...
                    $TwoHourButtonText
                    {
                        ...
                        if ($SelectedInterval.TotalSeconds -gt 0)
                        {
                            ...
                            Write-ADTLogEntry -Message "The user selected the two-hour deferral. The next prompt is scheduled for [$NextPromptTime]."
                        }
                        else
                        {
                            $NextPromptTime = $DeferDeadline
                    ...
                    $FourHourButtonText
                    {
                        ...
                        if ($SelectedInterval.TotalSeconds -gt 0)
                        {
                            ...
                            Write-ADTLogEntry -Message "The user selected the four-hour deferral. The next prompt is scheduled for [$NextPromptTime]."
                        }
                        else
                        {
                            $NextPromptTime = $DeferDeadline
                    ...
                    default
                    {
                        ...
                        {
                            Write-ADTLogEntry -Message "The initial deferral prompt was bypassed because the deployment entered [$($ADTSession.DeployMode)] mode. Continuing without deferral."

                            $ProceedWithInstallation = $true
                        }
                        else
                        {
                            Write-ADTLogEntry -Message "The initial prompt returned an unexpected result [$UserSelection]. The prompt will be displayed again after 60 seconds."
...
"@
            ...
            switch ($UserSelection)
            {
                ...
                $TwoHourButtonText
                {
                    ...
                    if ($SelectedInterval.TotalSeconds -gt 0)
                    {
                        ...
                        Write-ADTLogEntry -Message "The user selected another two-hour deferral. The next prompt is scheduled for [$NextPromptTime]."
                    }
                    else
                    {
                        $NextPromptTime = $DeferDeadline
                ...
                default
                {
                    ...
                    {
                        Write-ADTLogEntry -Message "The subsequent deferral prompt was bypassed because the deployment entered [$($ADTSession.DeployMode)] mode. Continuing without deferral."

                        $ProceedWithInstallation = $true
                    }
                    else
                    {
                        Write-ADTLogEntry -Message "The subsequent prompt returned an unexpected result [$UserSelection]. The prompt will be displayed again after 60 seconds."
        ...
        ##================================================
        ## MARK: Silent Application Closure
        ##================================================

        if (
            -not $FinalEnforcementDisplayed -and
        ...
        )
        {
            ...
            if ($RunningProcessesBeforeClosure.Count -gt 0)
            {
                ...
                Write-ADTLogEntry -Message "Silently closing configured process(es): [$RunningProcessNames]."

                $SilentCloseParams = @{
                    CloseProcesses = $ADTSession.AppProcessesToClose
                    Silent         = $true
                    Title          = "$($ADTSession.AppName) Install/Update"
                    Subtitle       = "Moody's - App Installation: Reboot required"
                }

                Show-ADTInstallationWelcome @SilentCloseParams

                Write-ADTLogEntry -Message 'Configured impacted processes were closed. Continuing with installation.'
            }
            else
            {
                Write-ADTLogEntry -Message 'No configured impacted processes are currently running. Continuing with installation.'
            }
        }
        else
        {
            ...
            {
                Write-ADTLogEntry -Message 'No processes are configured in AppProcessesToClose. Continuing with installation.'
            }
        }
    }
    catch
    {
        $ErrorMessage = "The managed reboot-deferral workflow failed. Error: $($_.Exception.Message)"

        Write-ADTLogEntry -Message $ErrorMessage `
